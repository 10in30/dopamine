...
`@dopamine/core` (web), `DopamineCore` (Swift), and `dopamine-core` (Android)
...
timing + factory + test, depends on `@dopamine/core`, self-registers on import;
...
vitest/Vite aliases pick it up; add it to the `@dopamine/effects` umbrella deps
...
> (a `[[buffer]]` vertex array, e.g. lightning) also author per-platform
> shaders today; `docs/roadmap.md` covers transpiling that logic.
