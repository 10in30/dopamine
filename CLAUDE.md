...
## Uniform binding — generated by the @dopamine/build toolchain (from the `.dope`)
...
drift. The build toolchain (`tools/dopamine`) makes that binding map **data**:
from each effect's `.dope` `render.params` + the `.dope` `binding` contract
(`excludeParams` / `scatterKey` / `extras` / `samplers` — which params are NOT
shader uniforms, the seed-keyed scatter field, the per-frame/host extras, the
texture samplers) it generates, from one source of truth, into the effect's
`dist/` SwiftPM package:
