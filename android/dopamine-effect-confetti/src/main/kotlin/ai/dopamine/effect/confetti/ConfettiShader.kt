// GLSL ES 3.00 source for Confetti — the web `confetti-shader.ts` reused
...
//
// One full-screen pass renders a burst of paper confetti as light: each piece
// LAUNCHES upward from `uOrigin` in a cone, then TUMBLES DOWN under gravity (the
// shared `ballisticPos`) with an air-drag FLUTTER sway and a per-axis SPIN that
// flashes the piece face-on (bright) vs edge-on (dim). Pieces settle + fade over
// their life (the shared `particleFade`). The `uShadow > 0.5` branch casts a
// downward silhouette on the (portable) multiply pass — kept for contract parity,
// unused by the single-surface overlay host.
...
val CONFETTI_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX
