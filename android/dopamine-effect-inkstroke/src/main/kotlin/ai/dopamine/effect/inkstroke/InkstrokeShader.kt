// GLSL ES 3.00 source for Inkstroke (Calligraphic Verdict) — the web
// `inkstroke-shader.ts` reused VERBATIM (Android OpenGL ES 3.0 speaks the same
// GLSL ES 3.00 as WebGL2). The shared "look" chunks come from `dopamine-core`
// (one canonical copy). The ONLY change from the web body is the final LIGHT-pass
// emit: `dopLightOut(col)` (premultiplied alpha = brightness) instead of
// `vec4(max(col, 0.0), 1.0)`, because the Android overlay is self-contained (no
// CSS screen-blend against the page — see Look.kt). The RGB look is byte-identical
// to web; the `uShadow > 0.5` branch is left unchanged for contract parity (the
// single-surface host renders light only).
//
// One full-screen pass renders, all summed as light: a faint paper wash, a
// two-leg checkmark brush path with pressure-modulated width + wet-ink bleed +
// bristle rake + a racing wet leading tip, flung droplets arcing under gravity,
// and an after-shimmer underline. A filmic ACES tonemap + a cel/neon whimsy pass
// + ordered dither finish the frame.
//
// `#define MAX_DROPS 64` is written as a literal; it must equal the integer-clamp
// const passed to the loader (`{ "MAX_DROPS" to 64.0 }` in Inkstroke.kt).
...
// Inkstroke's frag reads `gl_FragCoord`, so the standard fullscreen vertex
// (which also exposes `vUv`, unused here) is fine — matches the web.
val INKSTROKE_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX
