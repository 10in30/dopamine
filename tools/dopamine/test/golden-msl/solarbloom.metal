...
inline float4 dopMarkOut(float3 glow, float3 markInk, float markA, constant SolarbloomUniforms &u) {
  glow = max(glow, 0.0);
...
fragment float4 solarbloom_fragment(
  ...
  float inkL = dot(u.c0, float3(0.2126, 0.7152, 0.0722));
  float3 markInk = clamp(mix(float3(inkL), u.c0, 1.7) * 0.46, 0.0, 0.72);
  return dopMarkOut(col, markInk, markA, u);
}
