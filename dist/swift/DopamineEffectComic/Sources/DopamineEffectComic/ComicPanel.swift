...
// Text: the PANEL-DRAW SEAM. This is the ONLY hand-written Swift the effect
// ships; the factory shell (`Comic.swift`), the resource-bundle accessor, the
// MSL shader and the uniforms glue are all GENERATED from comic.dope.json + the
...
// consumes (amp/presence/flash) is `tempo.frame` DATA; the draw-side timing
// below (the per-letter slam SCALE + the presence the panel GEOMETRY needs)
// stays code by design — and the per-letter display-face LETTERING is genuinely
// code too (glyph layout is not datafied).
//
...
// fragment shader (the generated Comic.metal) samples that texture and adds the
// Ben-Day halftone / action lines / flash / pop-art look on top. The Swift
// backbone owns the panel runner (MetalOverlayHost builds + uploads it every
// frame); this file supplies ONLY the per-effect draw, a faithful port of
...
// / Luckiest Guy) is loaded from this package's Resources/fonts (ttf converted
...
// stacked outline / inkRoundness treatment — driven by the typography fields the
// loader composes into the resolved bag. Falls back to a bold system face if the
// face can't be loaded so the word still reads.
...
/// burst diameter ≈ 0.88·basis, so a comic ≈ 1.5× the element). Kept in sync
/// with the web comic renderer (`COMIC_TARGET_FILL`).
...
/// `.dope`; reskinning the word list is a `.dope` edit (this mirror only feeds
/// the host-side panel draw — the loader does not pick the word).
private let COMIC_WORD_POOL: [String] = ["YES!", "DONE!", "NICE!", "OKAY!", "WIN!", "GREAT!", "WOO!", "✓"]

// ── Draw-side tempo (the curves the panel GEOMETRY needs; the shader-facing
//    per-frame values ride `tempo.frame` in the `.dope`). Ports of the web
//    comic-tempo.ts. ──

/// Window (ms) over which the comic word SLAMS in.
private let IMPACT_MS: Double = 200
/// Hold (ms) the word sits proud at full size before it begins to settle out.
private let IMPACT_HOLD_MS: Double = 650

/// Comic impact SCALE over elapsed ms: large at t≈0, slamming to ≈1 by IMPACT_MS
/// (with a small spring), then resting. `overshoot` scales the slam magnitude.
private func impactScale(_ elapsedMs: Double, overshoot: Double = 1) -> Double {
    let t = elapsedMs
...
private func impactPresence(_ life: Double) -> Double {
...
/// host: a jagged starburst balloon (B fill + G outline) then the seed-picked
/// onomatopoeia word (R fill + G ink contour, full per-letter typography in the
/// mood face) or a vector checkmark, at the live slam pose.
public func drawComicPanel(
    ...
    let word = pickFromList(COMIC_WORD_POOL, seed: UInt32(truncatingIfNeeded: Int(rawSeed.rounded(.towardZero))))
