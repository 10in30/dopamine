...
inline float4 dopMarkOut(float3 glow, float3 markInk, float markA, constant HeartburstUniforms &u) {
  glow = max(glow, 0.0);
...
fragment float4 heartburst_fragment(
  ...
  float markA = clamp((heartFill + burstFill * burstFade + contour) * u.presence, 0.0, 1.0);
  float hbL = dot(u.c0, float3(0.2126, 0.7152, 0.0722));
  float3 markInk = clamp(mix(float3(hbL), u.c0, 1.5) * 0.52, 0.0, 0.72);
