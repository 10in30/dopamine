// @generated by @dopaminefx/build from effects/dots/dots.dope.json — do not edit.
//
// Dots as a Dopamine effect on the Swift backbone — the DATA-DRIVEN factory
// shell. This effect ships NO hand-written Swift: everything that isn't the
// shader lives in dots.dope.json — the mood→params mapping + OKLCH palette
...
// GLSL by the toolchain; `DotsUniforms` + `packDotsUniforms` are generated
// from the `.dope` binding contract.)

import Foundation
import DopamineCore

/// Dots: resolves a feeling → the flat `.dope` param bag. The drawable side
/// is Metal-only (below).
public final class Dots: EffectFactory {
    public let name = "dots"
    ...
    public init() throws {
        ...
        self.doc = try DopeResource.loadDope("dots.dope", bundle: .module)
    }
...
public extension Dots {
    ...
    /// shared backbone.
    static func passConfig() throws -> DopePassConfig<DotsUniforms> {
        try DopePassConfig(
            doc: DopeResource.loadDope("dots.dope", bundle: .module),
            vertexFunction: "dots_vertex",
            fragmentFunction: "dots_fragment",
            packUniforms: packDotsUniforms
        )
    }
}
#endif
