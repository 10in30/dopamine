...
## Installing via SwiftPM (published packages)

SwiftPM has no mainstream registry — a package is consumed by **git URL + a
semver tag**, and the URL always resolves the `Package.swift` at the repo
**root**. So the effects are published as ONE **aggregate** package (this repo,
root manifest) that exposes `DopamineCore` + every `DopamineEffect<Name>` as a
library product:
...
targets: [
    .target(name: "MyApp", dependencies: [
        ...
        .product(name: "DopamineEffectComic", package: "Dopamine"),
        // … any other DopamineEffect<Name> you use
    ]),
]
```

The aggregate root `Package.swift` is **generated** (`scripts/swift-aggregate.mjs`),
not committed to `main`: its effect target paths point into the gitignored
`dist/swift/` trees. The **`swift-release`** workflow
(`.github/workflows/swift-release.yml`, run manually with a version) builds
`dist/`, writes the manifest, and commits both onto a **detached release commit**
tagged `v<version>` — so a tag checkout is a complete, installable package while
`main` stays clean. To produce + test it locally:

```sh
node tools/dopamine/src/cli.mjs build      # generate dist/swift/*
node scripts/swift-aggregate.mjs           # write ./Package.swift (gitignored)
swift package dump-package                 # validate the manifest
```

The per-effect `dist/swift/DopamineEffect<Name>` packages remain standalone (the
iOS/macOS demo consumes them by path); the aggregate is the URL-installable face
of the same generated bytes.
