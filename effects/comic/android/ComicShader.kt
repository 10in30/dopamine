// GLSL ES 3.00 source for **Comic Impact** — the web `comic-shader.ts` reused
...
// alpha = brightness) instead of `vec4(max(col, 0.0), 1.0)`, because the Android
// overlay is self-contained (no CSS screen-blend against the page — see Look.kt).
// The RGB look is byte-identical to web.
//
// This is a HYBRID effect: the crisp blocky lettering, the jagged starburst, and
// the bold ink contours are drawn into an OFFSCREEN Canvas (ComicPanel.kt) and
// handed to this shader as one "panel" texture; the shader then does everything
// procedural + screen-space — Ben-Day halftone dots, radiating action / speed
// lines, the impact flash, and the NOIR ↔ POP-ART styling.
//
// Panel texture channel encoding (must match ComicPanel.kt):
...
// The web COMIC_VERTEX_SRC is the standard full-screen-triangle that emits `vUv`
// (`gl_VertexID` trick, no VBO) — identical to GLSL_FULLSCREEN_VERTEX.
val COMIC_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX

val COMIC_FRAGMENT_SRC: String = """#version 300 es
