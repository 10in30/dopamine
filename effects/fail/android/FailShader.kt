// GLSL ES 3.00 source for the FAIL / error effect — the web `fail-shader.ts`
// reused VERBATIM (Android OpenGL ES 3.0 speaks the same GLSL ES 3.00 as WebGL2).
// The shared "look" chunks come from `dopamine-core` (one canonical copy). The
// ONLY change from the web body is the final emit: `dopLightOut(col)`
...
// Look.kt). The RGB look is byte-identical to web.
//
// A red/amber ✗ cross is STAMPED in light over a tight, recoiling error flare;
// the whole frame desaturates and collapses fast. The shader declares the baked-✗
// SDF sampler (uSdfTex / uSdfOn) for portability, but the Android GL backbone has
// no aux-texture support, so uSdfOn stays 0 and the ANALYTIC two-bar ✗ renders (no
// texture binding needed). The vertex is GLSL_FULLSCREEN_VERTEX (the fragment uses
// gl_FragCoord directly and never reads vUv).
...
val FAIL_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX

val FAIL_FRAGMENT_SRC: String = """#version 300 es
