// GLSL ES 3.00 source for Ripple — the web `ripple-shader.ts` reused VERBATIM
...
// renders light only).
//
// One full-screen pass renders the tactile "droplet in a still pool", all summed
// as light: a WAVEFIELD of concentric travelling cosine wave packets launched in
// a stagger from the origin, CAUSTICS focused by the wave slope, a thin CREST
// GLINT riding each leading wavefront, all gated by a radial wavefront envelope so
// light lives only where a ring currently is, then the pool settles. A filmic
// ACES-ish tonemap + ordered dither finish the frame; a cel branch (whimsy) hard-
// posterizes the rings + caustics.
//
// `#define MAX_RINGS 7` is written as a literal; it must equal the integer-clamp
// const passed to the loader (`{ "MAX_RINGS": 7 }` in Ripple.kt) and the GLSL loop
// cap below.
...
// Ripple's frag reads `gl_FragCoord`, so the standard fullscreen vertex (which
// also exposes `vUv`, unused here) is fine — its `gl_Position` is identical to the
// web ripple vertex, matching the web. (Same situation as Solarbloom.)
val RIPPLE_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX
