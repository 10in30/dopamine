// GLSL ES 3.00 source for Solarbloom — the web `solarbloom-shader.ts` reused
...
// change from the web body is the final LIGHT-pass emit: `dopLightOut(col)`
// (premultiplied alpha = brightness) instead of `vec4(col, 1.0)`, because the
// Android overlay is self-contained (no CSS screen-blend against the page — see
// Look.kt). The RGB look is byte-identical to web; the `uShadow` branch is left
// unchanged for contract parity (the single-surface host renders light only).
//
// One full-screen pass renders, all summed as light: a volumetric domain-warped
// FBM bloom with angular shafts + spectral split + iridescent thin-film shimmer,
// depth-layered drifting light "motes" with motion-blur streaks + twinkle, and a
// checkmark drawn in light with a leading spark + afterglow. A filmic ACES-ish
// tonemap + ordered dither finish the frame.
//
// `#define MAX_MOTES 80` is written as a literal; it must equal the integer-clamp
// const passed to the loader (`{ "MAX_MOTES": 80 }` in Solarbloom.kt).
...
// Solarbloom's frag reads `gl_FragCoord`, so the standard fullscreen vertex
// (which also exposes `vUv`, unused here) is fine — matches the web.
val SOLARBLOOM_VERTEX_SRC: String = GLSL_FULLSCREEN_VERTEX
...
void main(){
  ...
  // instead of the web's opaque `vec4(col, 1.0)`. See Look.kt.
