...
// sweep), `render.shadowHeightFrac`, `render.consts` (MAX_CURTAINS),
...
// IGNORES the anchor (`render.config.usesOrigin = false`) — unlike the radial
// Solarbloom. The `.dope` bytes are the SAME as the web's (byte-parity proven).
...
class Aurora(context: Context) : DrawableEffect {
    ...
    // amp + sweep, shadow height, reduced motion) is data: aurora.dope.json
    ...
    private val config: PassConfig = dopePassConfig(doc, AURORA_VERTEX_SRC, AURORA_FRAGMENT_SRC, plan)
