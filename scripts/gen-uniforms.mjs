...
const STANDARD = [
  ...
  // Size (device px) of the underlying element the effect targets, centred on
  // `origin`. The centrepiece (checkmark glyph, ✗, comic word, hero heart) is
  // sized to THIS box so it matches the page element, not the whole canvas.
  // Defaults to `resolution` (the full canvas) when no element is targeted, so
  // full-page fires are unchanged.
...
const EFFECTS = {
  ...
  // NOTE: `comic` has MOVED out of this generator. It is the first effect ported
  // to the single-folder model (effects/comic/) built by the @dopamine/build
  // toolchain (tools/dopamine), which emits comic's Swift/MSL uniform structs into
  // its standalone dist/ package. The remaining effects are still generated here
